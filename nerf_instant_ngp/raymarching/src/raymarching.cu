...
__global__ void kernel_composite_rays_train_backward(
    ...
    while (step < num_steps) {
        ...
        grad_sigmas[0] = deltas[0] * (
            ...
            grad_weights_sum[0] * (1.0f - ws_final)
        );
...
__global__ void kernel_composite_rays(
    ...
    while (step < n_step) {
        ...
        */
        const scalar_t T = 1.0f - weight_sum;
        const scalar_t weight = alpha * T;
        weight_sum += weight;
...
template <typename scalar_t>
__global__ void kernel_composite_rays_rgb_geofeat(
    const uint32_t n_alive,
    const uint32_t n_step,
    const float T_thresh,
    int* rays_alive,
    ...
    const scalar_t* __restrict__ sigmas,
    const scalar_t* __restrict__ rgbs,
    const scalar_t* __restrict__ geofeat,
    const scalar_t* __restrict__ deltas,
    scalar_t* weights_sum, scalar_t* depth, scalar_t* image, scalar_t* geofeat_out
) {
    const uint32_t n = threadIdx.x + blockIdx.x * blockDim.x;
    if (n >= n_alive) return;
    ...
    constexpr int K = 15;  // geofeat dimension
    ...
    #pragma unroll
    for (int c = 0; c < K; ++c) {
        geofeat_out[c] = accum[c];
    }
}


void composite_rays_rgb_geofeat(const uint32_t n_alive, const uint32_t n_step, const float T_thresh, at::Tensor rays_alive, at::Tensor rays_t, at::Tensor sigmas, at::Tensor rgbs, at::Tensor geofeat, at::Tensor deltas, at::Tensor weights, at::Tensor depth, at::Tensor image, at::Tensor geofeat_out) {
    static constexpr uint32_t N_THREAD = 128;
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
    image.scalar_type(), "composite_rays_rgb_geofeat", ([&] {
        kernel_composite_rays_rgb_geofeat<<<div_round_up(n_alive, N_THREAD), N_THREAD>>>(n_alive, n_step, T_thresh, rays_alive.data_ptr<int>(), rays_t.data_ptr<scalar_t>(), sigmas.data_ptr<scalar_t>(), rgbs.data_ptr<scalar_t>(), geofeat.data_ptr<scalar_t>(), deltas.data_ptr<scalar_t>(), weights.data_ptr<scalar_t>(), depth.data_ptr<scalar_t>(), image.data_ptr<scalar_t>(), geofeat_out.data_ptr<scalar_t>());
    }));
}


template <typename scalar_t>
__global__ void kernel_composite_rays_geofeat(
    const uint32_t n_alive,
    const uint32_t n_step,
    const float    T_thresh,
    int*           rays_alive,
    scalar_t*      rays_t,
    const scalar_t* __restrict__ sigmas,
    const scalar_t* __restrict__ geofeat,      // [n_alive, n_step, 15]
    const scalar_t* __restrict__ deltas,
    scalar_t*      weights_sum,
    scalar_t*      depth,
    scalar_t*      geofeat_out                       // [n_rays, 15]
) {
    constexpr int K = 15;
    ...
    // locate per-alive-ray slabs
    sigmas += n * n_step;
    geofeat += n * n_step * K;
    deltas += n * n_step * 2;

    // advance outputs to this ray
    rays_t      += index;
    weights_sum += index;
    depth       += index;
    geofeat_out      += index * K;
...
void composite_rays_geofeat(
    ...
    at::Tensor     rays_t,
    at::Tensor     sigmas,
    at::Tensor     geofeat,    // shape [n_alive, n_step, 15]
    at::Tensor     deltas,
    at::Tensor     weights,
    at::Tensor     depth,
    at::Tensor     geofeat_out     // shape [n_rays, 15]
) {
    static constexpr uint32_t N_THREAD = 128;
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
        geofeat_out.scalar_type(), "composite_rays_geofeat", ([&] {
            kernel_composite_rays_geofeat<<<div_round_up(n_alive, N_THREAD), N_THREAD>>>(
                n_alive, n_step, T_thresh,
                rays_alive.data_ptr<int>(),
                rays_t.data_ptr<scalar_t>(),
                sigmas.data_ptr<scalar_t>(),
                geofeat.data_ptr<scalar_t>(),
                deltas.data_ptr<scalar_t>(),
                weights.data_ptr<scalar_t>(),
                depth.data_ptr<scalar_t>(),
                geofeat_out.data_ptr<scalar_t>()
            );
        })
    );
}
...
template <typename scalar_t>
__global__ void kernel_composite_rays_depth(
    const uint32_t n_alive,
    const uint32_t n_step,
    const float T_thresh,
    int* rays_alive,
...
void composite_rays_depth(const uint32_t n_alive, const uint32_t n_step, const float T_thresh, at::Tensor rays_alive, at::Tensor rays_t, at::Tensor sigmas, at::Tensor deltas, at::Tensor weights, at::Tensor depth) {
    static constexpr uint32_t N_THREAD = 128;
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
    sigmas.scalar_type(), "composite_rays_depth", ([&] {
        kernel_composite_rays_depth<<<div_round_up(n_alive, N_THREAD), N_THREAD>>>(n_alive, n_step, T_thresh, rays_alive.data_ptr<int>(), rays_t.data_ptr<scalar_t>(), sigmas.data_ptr<scalar_t>(), deltas.data_ptr<scalar_t>(), weights.data_ptr<scalar_t>(), depth.data_ptr<scalar_t>());
    }));
}


// ----------------------------------------------
// Forward march + composite for K=15 geofeat channels
// ----------------------------------------------
template <typename scalar_t>
__global__ void kernel_composite_rays_train_forward_geofeat(
    const scalar_t* __restrict__ sigmas,   // [M]
    const scalar_t* __restrict__ geofeat,   // [M,15]
    const scalar_t* __restrict__ deltas,   // [M,2]
    const int*      __restrict__ rays,     // [N,3] = (idx,offset,num_steps)
    const uint32_t  M,
    const uint32_t  N,
    const float     T_thresh,
    scalar_t*       weights_sum,           // [N]
    scalar_t*       depth,                 // [N]
    scalar_t*       output                 // [N,15]
) {
    constexpr int K = 15;
    ...
    // unpack ray metadata
    uint32_t idx       = rays[n*3 + 0];
    uint32_t offset    = rays[n*3 + 1];
    uint32_t num_steps = rays[n*3 + 2];

    // empty or out‑of‑bounds
    if (num_steps == 0 || offset + num_steps > M) {
        weights_sum[idx] = depth[idx] = scalar_t(0);
        #pragma unroll
        for (int c = 0; c < K; ++c) {
            output[idx*K + c] = scalar_t(0);
        }
        return;
    }

    // advance pointers into the big arrays
    sigmas += offset;
    geofeat += offset*K;
    deltas += offset*2;

    // accumulators
    scalar_t T  = scalar_t(1);
    scalar_t ws = scalar_t(0);
    scalar_t t  = scalar_t(0);
    scalar_t d  = scalar_t(0);
    scalar_t acc[K];
    #pragma unroll
    for (int c = 0; c < K; ++c) {
        acc[c] = scalar_t(0);
    ...
    for (uint32_t step = 0; step < num_steps; ++step) {
        const scalar_t alpha = scalar_t(1) - __expf(-sigmas[0] * deltas[0]);
        const scalar_t w = alpha * T;
    ...
    #pragma unroll
    for (int c = 0; c < K; ++c) {
        output[idx*K + c] = acc[c];
    }
}


void composite_rays_train_forward_geofeat(
    const at::Tensor sigmas,   // [M]
    const at::Tensor geofeat,   // [M,15]
    const at::Tensor deltas,   // [M,2]
    const at::Tensor rays,     // [N,3]
    const uint32_t    M,
    const uint32_t    N,
    const float       T_thresh,
    at::Tensor weights_sum, // [N]
    at::Tensor depth,       // [N]
    at::Tensor output       // [N,15]
) {
    static constexpr uint32_t N_THREAD = 128;
    dim3 grid((N + N_THREAD - 1) / N_THREAD), block(N_THREAD);

    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      sigmas.scalar_type(), "composite_rays_train_forward_geofeat", ([&] {
        kernel_composite_rays_train_forward_geofeat<scalar_t><<<grid,block>>>(
          sigmas.data_ptr<scalar_t>(),
          geofeat.data_ptr<scalar_t>(),
          deltas.data_ptr<scalar_t>(),
          rays.data_ptr<int>(),
...
// ----------------------------------------------------
// Backward pass: gradients wrt sigmas and 15-channel geofeats
// ----------------------------------------------------
template <typename scalar_t>
__global__ void kernel_composite_rays_train_backward_geofeat(
    const scalar_t* __restrict__ grad_ws,    // [N]
    const scalar_t* __restrict__ grad_out,   // [N,15]
    const scalar_t* __restrict__ sigmas,     // [M]
    const scalar_t* __restrict__ geofeat,     // [M,15]
    const scalar_t* __restrict__ deltas,     // [M,2]
    const int*      __restrict__ rays,       // [N,3]
    const scalar_t* __restrict__ ws_sum,     // [N]
    const scalar_t* __restrict__ out,        // [N,15]
    uint32_t        M,
    uint32_t        N,
    float           T_thresh,
    scalar_t*       grad_sigmas,             // [M]
    scalar_t*       grad_geofeat              // [M,15]
) {
    constexpr int K = 15;
    uint32_t n = threadIdx.x + blockIdx.x*blockDim.x;
    if (n >= N) return;
    ...
    deltas       += offset*2;
    grad_sigmas  += offset;
    grad_geofeat += offset*K;

    // finals
    scalar_t T       = scalar_t(1);
    scalar_t ws_f    = ws_sum[0];
    scalar_t final[K];
    #pragma unroll
    for (int c = 0; c < K; ++c) {
        final[c] = out[c];
    ...
    for (uint32_t step = 0; step < num_steps; ++step) {
        ...
        for (int c = 0; c < K; ++c) {
            acc[c] += w * geofeat[c];
            // grad wrt geofeat
            grad_geofeat[c] = grad_out[c] * w;
        }
        ws += w;

        // compute grad_sigma term = δt * ( Σ_c ∂L/∂out[c] * ( T*geofeat[c] - (final[c] - acc[c]) ) + ∂L/∂ws * (1 - ws_f) )
        scalar_t inner = grad_ws[0] * (scalar_t(1) - ws_f);
        #pragma unroll
        for (int c = 0; c < K; ++c) {
            inner += grad_out[c] * (T*geofeat[c] - (final[c] - acc[c]));
        }
        grad_sigmas[0] = deltas[0] * inner;
...
void composite_rays_train_backward_geofeat(
    const at::Tensor grad_ws,     // [N]
    const at::Tensor grad_out,    // [N,15]
    const at::Tensor sigmas,      // [M]
    const at::Tensor geofeat,      // [M,15]
    const at::Tensor deltas,      // [M,2]
    const at::Tensor rays,        // [N,3]
    const at::Tensor ws_sum,      // [N]
    const at::Tensor out,         // [N,15]
    const uint32_t    M,
    const uint32_t    N,
    const float       T_thresh,
    at::Tensor grad_sigmas, // [M]
    at::Tensor grad_geofeat  // [M,15]
) {
    static constexpr uint32_t N_THREAD = 128;
    dim3 grid((N + N_THREAD - 1) / N_THREAD), block(N_THREAD);

    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      grad_out.scalar_type(), "composite_rays_train_backward_geofeat", ([&] {
        kernel_composite_rays_train_backward_geofeat<scalar_t><<<grid,block>>>(
          grad_ws.data_ptr<scalar_t>(),
          grad_out.data_ptr<scalar_t>(),
          sigmas.data_ptr<scalar_t>(),
          geofeat.data_ptr<scalar_t>(),
          deltas.data_ptr<scalar_t>(),
          rays.data_ptr<int>(),
          ws_sum.data_ptr<scalar_t>(),
          out.data_ptr<scalar_t>(),
          M, N, T_thresh,
          grad_sigmas.data_ptr<scalar_t>(),
          grad_geofeat.data_ptr<scalar_t>()
        );
      })
    );
}