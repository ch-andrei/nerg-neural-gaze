...
class _composite_rays_train_geofeat(Function):
    @staticmethod
    @custom_fwd(cast_inputs=torch.float32)
    def forward(ctx, sigmas, geofeat, deltas, rays, T_thresh=1e-4):
        ...
        image = torch.empty(N, K, dtype=sigmas.dtype, device=sigmas.device)

        _backend.composite_rays_train_forward_geofeat(sigmas, geofeat, deltas, rays, M, N, T_thresh, weights_sum, depth, image)

        ctx.save_for_backward(sigmas, geofeat, deltas, rays, weights_sum, depth, image)
    ...
    def backward(ctx, grad_weights_sum, grad_depth, grad_image):
        ...
        sigmas, geofeat, deltas, rays, weights_sum, depth, image = ctx.saved_tensors
        M, N, T_thresh = ctx.dims

        grad_sigmas = torch.zeros_like(sigmas)
        grad_geofeat = torch.zeros_like(geofeat)

        _backend.composite_rays_train_backward_geofeat(
            grad_weights_sum, grad_image, sigmas, geofeat, deltas, rays, weights_sum, image, M, N, T_thresh,
            grad_sigmas, grad_geofeat)

        return grad_sigmas, grad_geofeat, None, None, None


composite_rays_train_geofeat = _composite_rays_train_geofeat.apply
...
class _composite_rays_geofeat(Function):
    @staticmethod
    @custom_fwd(cast_inputs=torch.float32) # need to cast sigmas & geofeat to float
    def forward(ctx, n_alive, n_step, rays_alive, rays_t, sigmas, geofeat, deltas, weights_sum, depth, geofeat_out, T_thresh=1e-2):
        ''' composite rays' geofeat, according to the ray marching formula. (for inference)
        Args:
            n_alive: int, number of alive rays
            n_step: int, how many steps we march
            ...
            geofeat: float, [n_alive * n_step, 15]
            deltas: float, [n_alive * n_step, 2], all generated points' deltas (here we record two deltas, the first is for RGB, the second for depth).
        In-place Outputs:
            weights_sum: float, [N,], the alpha channel
            depth: float, [N,], the depth value
            geofeat_out: float, [N, 15], the RGB channel (after multiplying alpha!)
        '''
        _backend.composite_rays_geofeat(n_alive, n_step, T_thresh, rays_alive, rays_t, sigmas, geofeat, deltas, weights_sum, depth, geofeat_out)
        return tuple()


composite_rays_geofeat = _composite_rays_geofeat.apply


class _composite_rays(Function):
    @staticmethod
    @custom_fwd(cast_inputs=torch.float32) # need to cast sigmas & rgbs to float
    def forward(ctx, n_alive, n_step, rays_alive, rays_t, sigmas, rgbs, geofeats, deltas, weights_sum, depth, image, geofeat_out, T_thresh=1e-2, return_geofeat=False):
        ...
        In-place Outputs:
            ...
            depth: float, [N,], the depth value
            image: float, [N, 3], the RGB channel (after multiplying alpha!)
            geofeat_out: float, [N, 15], geofeats
        '''
        if return_geofeat:
            # also return accumulated geofeats
            _backend.composite_rays_rgb_geofeat(n_alive, n_step, T_thresh, rays_alive, rays_t, sigmas, rgbs, geofeats, deltas, weights_sum, depth, image, geofeat_out)
        else:
...
class _composite_rays_depth(Function):
    @staticmethod
    @custom_fwd(cast_inputs=torch.float32) # need to cast sigmas & rgbs to float
    def forward(ctx, n_alive, n_step, rays_alive, rays_t, sigmas, deltas, weights_sum, depth, T_thresh=1e-2):
        ''' same as _composite_rays but only computes depth, used for more efficient depth computation.
        Args:
            n_alive: int, number of alive rays
            n_step: int, how many steps we march
            rays_alive: int, [n_alive], the alive rays' IDs in N (N >= n_alive)
        ...
        In-place Outputs:
            ...
            depth: float, [N,], the depth value
        '''
        _backend.composite_rays_depth(n_alive, n_step, T_thresh, rays_alive, rays_t, sigmas, deltas, weights_sum, depth)
        return tuple()


composite_rays_depth = _composite_rays_depth.apply
