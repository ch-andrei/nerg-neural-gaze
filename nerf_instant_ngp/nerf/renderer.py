...
import nerf_instant_ngp.raymarching as raymarching
...
class NeRFRenderer(nn.Module):
    ...
    def run(self, rays_o, rays_d, num_steps=128, upsample_steps=128, bg_color=None, perturb=False, **kwargs):
        ...
        # query SDF and RGB
        density_outputs = self.density(xyzs.reshape(-1, self.output_dim))
        ...
        signal = self.color(xyzs.reshape(-1, 3), dirs.reshape(-1, 3), mask=mask.reshape(-1), **density_outputs)
        signal = signal.view(N, -1, 3)  # [N, T+t, 3]
        ...
        # calculate color
        output_signal = torch.sum(weights.unsqueeze(-1) * signal, dim=-2)  # [N, self.output_dim], in [0, 1]

        # mix background color
        if self.bg_radius > 0:
            # use the bg model to calculate bg_color
            sph = raymarching.sph_from_ray(rays_o, rays_d, self.bg_radius)  # [N, 2] in [-1, 1]
            bg_color = self.background(sph, rays_d.reshape(-1, 3))  # [N, self.output_dim]
        elif bg_color is None:
            bg_color = 1

        output_signal = output_signal + (1 - weights_sum).unsqueeze(-1) * bg_color

        output_signal = output_signal.view(*prefix, 3)
    ...
    def run_cuda_depth_geofeat(self, rays_o, rays_d, dt_gamma=0, perturb=False, max_steps=1024, T_thresh=1e-4,
                               return_geofeat=False):
        # this function only evaluates depth and geometric features along the queried rays (but not the final color)
        ...
        if return_geofeat:
            geo_feat_out = torch.zeros(N, self.geo_feat_dim, dtype=dtype, device=device)
        ...
        while step < max_steps:
            ...
            xyzs, _, deltas = raymarching.march_rays(n_alive, n_step, rays_alive, rays_t, rays_o, rays_d,
            ...
            outputs = self.density(xyzs)
            sigmas = outputs["sigma"]
            sigmas = self.density_scale * sigmas

            if return_geofeat:
                # NeRF-like volume rendering to accumulate depth and geofeat (but not color)
                geo_feat = outputs["geo_feat"]
                raymarching.composite_rays_geofeat(n_alive, n_step, rays_alive, rays_t, sigmas, geo_feat, deltas,
                                                   weights_sum, depth, geo_feat_out, T_thresh),
            else:
                # NeRF-like volume rendering to accumulate only depth (but not color and no geofeat)
                raymarching.composite_rays_depth(n_alive, n_step, rays_alive, rays_t, sigmas, deltas, weights_sum,
                                                 depth, T_thresh)
        ...
        results = {
            "depth": depth.view(*prefix)  # depth in world space coords (no rescaling to 0-1 for visualization)
        }

        if return_geofeat:
            results['geofeat'] = geo_feat_out.view(*prefix, self.geo_feat_dim)

        return results

    def run_cuda(self, rays_o, rays_d, dt_gamma=0, bg_color=None, perturb=False, force_all_rays=False, max_steps=1024,
                 T_thresh=1e-4, return_geofeat=False, **kwargs):
        # rays_o, rays_d: [B, N, 3], assumes B == 1
        # return: image: [B, N, 3], depth: [B, N]
        ...
        if self.bg_radius > 0:
            # use the bg model to calculate bg_color
            sph = raymarching.sph_from_ray(rays_o, rays_d, self.bg_radius)  # [N, 2] in [-1, 1]
            bg_color = self.background(sph, rays_d)  # [N, self.output_dim]
        elif bg_color is None:
            bg_color = 1
        ...
        if self.training:
            if return_geofeat:
                raise NotImplementedError("Return geofeat not implemented for training mode.")
            ...
            weights_sum, depth, rgbs_out = raymarching.composite_rays_train(sigmas, rgbs, deltas, rays, T_thresh)

            rgbs_out = rgbs_out + (1 - weights_sum).unsqueeze(-1) * bg_color
            depth_w = depth.view(*prefix)  # depth in world space (not scaled to 0-1 for visualization)
            depth = torch.clamp(depth - nears, min=0) / (fars - nears)
            rgbs_out = rgbs_out.view(*prefix, 3)
            depth = depth.view(*prefix)
            ...
            weights_sum = torch.zeros(N, dtype=dtype, device=device)
            depth = torch.zeros(N, dtype=dtype, device=device)
            rgbs_out = torch.zeros(N, 3, dtype=dtype, device=device)
            geofeat_out = torch.zeros(N, self.geo_feat_dim, dtype=dtype, device=device) if return_geofeat else None
            ...
            while step < max_steps:
                ...
                sigmas, rgbs, geofeat = self(xyzs, dirs)
                ...
                raymarching.composite_rays(n_alive, n_step, rays_alive, rays_t, sigmas, rgbs, geofeat, deltas,
                                           weights_sum, depth, rgbs_out, geofeat_out, T_thresh, return_geofeat)
            ...
            if return_geofeat:
                geofeat_out = geofeat_out.view(*prefix, self.geo_feat_dim)

        results['depth_w'] = depth_w
        results['depth'] = depth
        results['image'] = rgbs_out

        if return_geofeat:
            results['geofeat'] = geofeat_out
