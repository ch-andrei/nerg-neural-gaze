...
class Trainer(object):
    def __init__(self,
        ...
        if opt.rand_pose >= 0: # =0 means only using CLIP loss, >0 means a hybrid mode.
            from nerf_instant_ngp.nerf.clip_utils import CLIPLoss
    ...
    def test_step(self, data, bg_color=None, perturb=False, model=None, return_geofeat=False):
        rays_o = data['rays_o'] # [B, N, 3]
        rays_d = data['rays_d'] # [B, N, 3]
        H, W = data['H'], data['W']
        ...
        outputs = model.render(rays_o, rays_d, staged=True, bg_color=bg_color, perturb=perturb, return_geofeat=return_geofeat, **vars(self.opt))
        ...
        if return_geofeat:
            pred_geofeat = outputs['geofeat'].reshape(-1, H, W, model.geo_feat_dim)
            return pred_rgb, pred_depth, pred_depth_w, pred_geofeat
        else:
            return pred_rgb, pred_depth, pred_depth_w
    ...
    def test(self, loader, save_path=None, name=None, write_video=True):
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(loader):
                ...
                gt_image = data["images"][0].detach().cpu().numpy()
                ...
                if write_video:
                    ...
                    cv2.imwrite(os.path.join(save_path, f'{name}_{i:04d}_rgb_gt.png'), cv2.cvtColor(gt_image, cv2.COLOR_RGB2BGR))
    ...
    def test_gui(self, pose, intrinsics, W, H, bg_color=None, spp=1, downscale=1):
        ...
        with torch.no_grad():
            with torch.cuda.amp.autocast(enabled=self.fp16):
                ...
                preds, preds_depth, preds_depth_w = self.test_step(data, bg_color=bg_color, perturb=False if spp == 1 else spp)
        ...
        if downscale != 1:
            ...
            preds_depth_w = F.interpolate(preds_depth_w.unsqueeze(1), size=(H, W), mode='nearest').squeeze(1)
        ...
        pred = preds[0].detach().cpu().numpy()
        pred_depth = preds_depth[0].detach().cpu().numpy()
        preds_depth_w = preds_depth_w[0].detach()  # this needs to be a torch tensor for nerg-gui

        outputs = {
            # tensors
            'data': data,
            'depth_w': preds_depth_w,  # depth in world coordinates (not relative to nears/fars)
            # ndarrays
            'image': pred,
            'depth': pred_depth,
        }
    ...
    def train_one_epoch(self, loader):
        ...
        for data in loader:
            ...
            if isinstance(loss, dict):
                ...
                loss_dict = {loss_name: loss.item() for loss_name, loss in loss.items() if loss_name != 'loss'}
                loss = loss['loss']
            ...
            if self.local_rank == 0:
                ...
                if self.scheduler_update_every_step:
                    pbar.set_description(f"loss={loss_val:.4f} ({total_loss/self.local_step:.4f}) [{loss_dict}], lr={self.optimizer.param_groups[0]['lr']:.6f}")
                else:
                    pbar.set_description(f"loss={loss_val:.4f} ({total_loss/self.local_step:.4f}) [{loss_dict}]")
                pbar.update(loader.batch_size)
        ...
        self.log(f"==> Finished Epoch {self.epoch} (avg_loss={average_loss:.4f}).")
    ...
    def evaluate_one_epoch(self, loader, name=None):
        ...
        with torch.no_grad():
            ...
            for data in loader:
                ...
                if self.local_rank == 0:
                    ...
                    pbar.set_description(f"loss={loss_val:.4f} [{loss_dict}] ({total_loss/self.local_step:.4f})")
                    pbar.update(loader.batch_size)
        ...
        self.log(f"++> Evaluate epoch {self.epoch} Finished (average_loss={average_loss}).")
    ...
    def load_checkpoint(self, checkpoint=None, model_only=False, checkpoint_func=None):
        if checkpoint is None:
            checkpoint_path = f'{self.ckpt_path}/{self.name}_ep*.pth'
            checkpoint_list = sorted(glob.glob(checkpoint_path))
            print('checkpoint_path', checkpoint_path, 'checkpoint_list', checkpoint_list)
            if checkpoint_list:
        ...
        if checkpoint_func is not None:
            print('WARNING: load_checkpoint running checkpoint_func...')
            checkpoint_func(checkpoint_dict)
