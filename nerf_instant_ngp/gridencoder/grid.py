...
class GridEncoder(nn.Module):
    def __init__(self, input_dim=3, num_levels=16, level_dim=2, per_level_scale=2, base_resolution=16, log2_hashmap_size=19, desired_resolution=None, gridtype='hash', align_corners=False, interpolation='linear', init_normal=False, init_std=1e-4, init_mean=0):
        super().__init__()
        ...
        self.reset_parameters(init_normal, init_mean, init_std)

    def reset_parameters(self, normal_distrib=False, mean=0, std=1e-4):
        if normal_distrib:
            self.embeddings.data.normal_(mean, std)
        else:
            self.embeddings.data.uniform_(-std-mean, std-mean)
